...
\urlstyle{same}
\hypersetup{
  pdftitle={Apparent Machine Learning Skill in Indian Dengue Forecasting Vanishes Under Leakage-Free Within-State Evaluation},
  pdfauthor={Siddalingaiah H S\^{}\{1,*\}; Sowjanya D\^{}\{1\}; Rangaswamy H V\^{}\{1\}},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Apparent Machine Learning Skill in Indian Dengue Forecasting
...
routinely report high accuracy, yet such claims depend entirely on the
provenance of the underlying surveillance data and on whether the
evaluation design excludes information unavailable at the moment a
forecast is issued. No continuous monthly state-level case series is
publicly available for India. \textbf{Objectives:} To assemble and
independently verify an authentic Indian state-level dengue panel; to
establish, under a strictly leakage-free design, how much genuine
one-year-ahead skill is attainable and whether machine learning improves
on elementary baselines; to quantify how far common design defects
inflate apparent performance; and to test whether structural indicators
explain the between-state burden landscape. \textbf{Methods:} We
compiled every observed annual state-level total released by India's
National Centre for Vector Borne Diseases Control (NCVBDC) and archived
by OpenDengue for 2015--2024, retaining only states with an observed
value in all ten years; nothing was interpolated, imputed or
reconstructed. Predictors for year \emph{t} used only information
observable by 31 December \emph{t}−1. Seven specifications ---
...
expanding-window splits. We report pooled and within-state
(state-mean-removed) R², out-of-fold permutation importance, and, for
outbreak-year classification, discrimination alongside calibration, with
all thresholds derived inside each training window. \textbf{Results:}
The panel comprises \textbf{350 state-years across 35 states and union
territories}; all 124 state-years overlapping the independent NCVBDC
bulletin matched \textbf{exactly}. No model beat elementary baselines:
state climatology achieved the highest pooled R² (\textbf{0.637}), ahead
of a negative-binomial GLM (0.607), gradient boosting (0.551) and
persistence (0.550). \textbf{Within-state R² was at or below zero for
every model} (best −0.002; gradient boosting −0.239), showing that
pooled skill reflects only the recovery of stable between-state
differences, not anticipation of temporal change. Outbreak-year
classification was uninformative (AUC 0.522; average precision 0.468
against a prevalence of 0.452; Brier 0.326, worse than the 0.248
non-informative baseline). Reintroducing an unshifted three-year rolling
mean raised apparent R² to \textbf{0.759}. By contrast, incidence rose
with urbanisation (ρ = \textbf{0.635}, p \textless{} 0.001) and GDP per
capita (ρ = \textbf{0.609}, p \textless{} 0.001), the latter robust to
adjustment for a surveillance-capacity proxy; the NITI Health Index
showed no association (ρ = 0.159). \textbf{Conclusions:} On
...
average, and reported accuracies well above this level should be treated
as evidence of design leakage rather than epidemiological signal. Burden
is instead patterned spatially, concentrating in more urbanised and
economically developed states. Surveillance and vector-control
investment should therefore be allocated on structural grounds rather
than on short-horizon algorithmic forecasting at this resolution.
\textbf{Keywords:} Dengue; India; Data leakage; Forecast evaluation;
Benchmarking; Negative results; Surveillance data provenance;
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 5. Between-state structural gradient in observed mean annual dengue incidence per 100 000 (log scale), 2015--2024, against (A) urban population share, (B) GDP per capita and (C) the NITI Aayog Health Index, with Spearman ρ, bootstrap 95\% CI and p-value.}]{figures/Figure5.png}}
